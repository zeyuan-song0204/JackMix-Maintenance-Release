...
//	init();
	...
	QStringList outs;
	{
		...
		if ( !yes ) {
			if(cn==1){
				ins = QStringList() << "输入_1" << "输入_2" << "输入_3" << "输入_4" << "输入_5" << "输入_6" << "输入_7" << "输入_8";
				outs = QStringList() << "输出_1" << "输出_2";
			}else{
	...
	if ( ins.empty() || outs.empty() ){
		if(cn==1){
			statusBar()->showMessage( "找不到频道:-(" );
		}else if(cn==2){
			statusBar()->showMessage( "Impossible de trouver la chaîne:-(" );
		}
		else{
...
MainWindow::MainWindow( QString filename, QWidget* p ) : QMainWindow( p ), _backend( new JackBackend( new GUI::GraphicalGuiServer( this ) ) ), _autofillscheduled( true ) {
	...
	QTranslator t;
	if(cn==1){t.load("chinese.qm");init_CN();}else if(cn==2){t.load("french.qm");init_FR();}
	else{init();}
	//init();
...
_filemenu->addAction( "Open File...", this, SLOT( openFile() ), Qt::CTRL+Qt::Key_O );
	...
	//_filemenu->addAction("&Quit", this,SLOT(close()),Qt::CTRL+Qt::Key_X);
...
//	_languagemenu=menuBar()->addMenu("&Language");
//	_languagemenu->addAction("Chinese",this,SLOT(toChinese()));
	...
	// _mw->layout->addWidget(button1);
	// connect(button1,SIGNAL(clicked()),this,SLOT(mute_all()));
...
void MainWindow::togglein() {
	...
	if ( shown ) {
		_inputswidget->hide();
		if(cn==1){
			_togglein_action->setText( "显示输入控制台" );
		}else if(cn==2){
			_togglein_action->setText( "Afficher la console d'entrée" );
		}
		else{_togglein_action->setText( "Show inputcontrols" );}
	} else {
		_inputswidget->show();
		if(cn==1){
                        _togglein_action->setText( "隐藏输入控制台" );
                }else if(cn==2){
					 _togglein_action->setText( "Masquer la console de saisie" );
				}
				else{_togglein_action->setText( "Hide inputcontrols" );}
	}
}
void MainWindow::toggleout() {
	...
	if ( shown ) {
		_outputswidget->hide();
		if(cn==1){
                        _togglein_action->setText( "显示输出控制台" );
                }else if(cn==2){
					_togglein_action->setText( "Afficher la console de sortie" );
				}
				else{_togglein_action->setText( "Show outputcontrols" );}
	} else {
		_outputswidget->show();
		if(cn==1){
                        _togglein_action->setText( "隐藏输出控制台" );
                }else if(cn==2){
					_togglein_action->setText( "Masquer la console de sortie" );
				}else{_togglein_action->setText( "Hide outputcontrols" );}
	}
}
...
void MainWindow::renameInput() {
	...
	JackMix::GUI::EditableChannelSelector *tmp =
                new JackMix::GUI::EditableChannelSelector("重命名输入频道",
                                                          "编辑频道名称",
                                                          _backend->inchannels(),
                                                          this);
	...
	JackMix::GUI::EditableChannelSelector *tmp =
                new JackMix::GUI::EditableChannelSelector("Renommer le canal d'entrée",
...
void MainWindow::renameOutput() {
	if(cn==1){
	JackMix::GUI::EditableChannelSelector *tmp =
                new JackMix::GUI::EditableChannelSelector("重命名输出频道",
                                                          "编辑频道名称",
	...
	JackMix::GUI::EditableChannelSelector *tmp =
                new JackMix::GUI::EditableChannelSelector("Renommer le canal de sortie",
                                                          "Modifier le nom de la chaîne",
                                                          _backend->outchannels(),
                                                          this);
	connect( tmp, SIGNAL(editedChannels(QStringList)), this, SLOT(renameOutput(QStringList)) );
...
void MainWindow::removeInput() {
	...
	JackMix::GUI::ChannelSelector *tmp =
                new JackMix::GUI::ChannelSelector( "删除输入频道",
                                                   "选择要删除的输入频道:",
                                                   _backend->inchannels(),
                                                   this );
	connect( tmp, SIGNAL( selectedChannel( QString ) ),
		...
		JackMix::GUI::ChannelSelector *tmp =
                new JackMix::GUI::ChannelSelector( "Supprimer le canal d'entrée",
                                                   "Sélectionnez le canal d'entrée à supprimer:",
                                                   _backend->inchannels(),
                                                   this );
	connect( tmp, SIGNAL( selectedChannel( QString ) ),
                 this, SLOT( removeInput( QString ) ) );
...
void MainWindow::removeOutput() {
	...
	if(cn==1){
	JackMix::GUI::ChannelSelector *tmp =
                new JackMix::GUI::ChannelSelector( "删除输出频道",
                                                   "选择要删除的频道:",
	...
	JackMix::GUI::ChannelSelector *tmp =
                new JackMix::GUI::ChannelSelector( "Supprimer le canal de sortie",
                                                   "Sélectionnez la chaîne à supprimer:",
                                                   _backend->outchannels(), this );
	connect( tmp, SIGNAL( selectedChannel( QString ) ),
                 this, SLOT( removeOutput( QString ) ) );
        connect( tmp, SIGNAL(finished(int)), tmp, SLOT(deleteLater()) );
...
void MainWindow::init_CN() {

        JackMix::MixerElements::init_aux_elements_CN();
        JackMix::MixerElements::init_stereo_elements_CN();

        _filemenu = menuBar()->addMenu("&文件" );
        _filemenu->addAction( "&打开文件...", this, SLOT( openFile() ), Qt::CTRL);
	_filemenu->addAction( "&保存文件...", this, SLOT( saveFile() ), Qt::CTRL);
        _filemenu->addSeparator();
	_filemenu->addAction( "&退出", this, SLOT( close() ), Qt::CTRL+Qt::Key_Q);

	_editmenu = menuBar()->addMenu( "&编辑" );
        _select_action = new QAction( "选择模式", this );
        _select_action->setCheckable( true );
        connect( _select_action, SIGNAL( triggered() ), this, SLOT( toggleselectmode() ) );
        //_editmenu->addAction( _select_action );
        //_select_action->addTo( new QToolBar( this ) );
        _editmenu->addAction( "&自动填写空位置", this, SLOT( scheduleAutoFill() ) );
        _editmenu->addSeparator();
        _add_inchannel_action = new QAction( "增添&输入端口...", this );
        connect( _add_inchannel_action, SIGNAL( triggered() ), this, SLOT( addInput() ) );
        _editmenu->addAction( _add_inchannel_action );
        _add_outchannel_action = new QAction( "增添&输出端口...", this );
        connect( _add_outchannel_action, SIGNAL( triggered() ), this, SLOT( addOutput() ) );
        _editmenu->addAction( _add_outchannel_action );
        _rename_input_action = new QAction( "&重命名输入端口...", this);
        connect( _rename_input_action, SIGNAL( triggered() ), this, SLOT(renameInput()) );
	_editmenu->addAction( _rename_input_action );
        _rename_output_action = new QAction( "&重命名输出端口...", this);
        connect( _rename_output_action, SIGNAL( triggered() ), this, SLOT(renameOutput()) );
        _editmenu->addAction( _rename_output_action );
        _remove_inchannel_action = new QAction( "&移除输入端口...", this );
        connect( _remove_inchannel_action, SIGNAL( triggered() ), this, SLOT( removeInput() ) );
        _editmenu->addAction( _remove_inchannel_action );
        _remove_outchannel_action = new QAction( "&移除输出端口...", this );
        connect( _remove_outchannel_action, SIGNAL( triggered() ), this, SLOT( removeOutput() ) );
        _editmenu->addAction( _remove_outchannel_action );

	_viewmenu = menuBar()->addMenu( "&视图" );
        _togglein_action = new QAction( "隐藏 &输入端口控制台", this );
        connect( _togglein_action, SIGNAL( triggered() ), this, SLOT( togglein() ) );
        _viewmenu->addAction( _togglein_action );
        _toggleout_action = new QAction( "隐藏 &输出端口控制台", this );
        connect( _toggleout_action, SIGNAL( triggered() ), this, SLOT( toggleout() ) );
        _viewmenu->addAction( _toggleout_action );

        _helpmenu = menuBar()->addMenu( "&帮助" );
        _helpmenu->addAction( "关于 &JackMix", this, SLOT( about() ) );
        _helpmenu->addAction( "关于 &Qt", this, SLOT( aboutQt() ) );
...
/*
        ...
        //_lashclient->setJackName( "JackMix" );
*/
        midiControlSender = new MidiControl::ControlSender("JackMix 控制台");
...
void MainWindow::init_FR() {

        JackMix::MixerElements::init_aux_elements_FR();
        JackMix::MixerElements::init_stereo_elements_FR();

        _filemenu = menuBar()->addMenu("&document" );
        _filemenu->addAction( "&ouvrir un fichier...", this, SLOT( openFile() ), Qt::CTRL);
	_filemenu->addAction( "&enregistrer le document...", this, SLOT( saveFile() ), Qt::CTRL);
        _filemenu->addSeparator();
	_filemenu->addAction( "&quitter", this, SLOT( close() ), Qt::CTRL+Qt::Key_Q);

	_editmenu = menuBar()->addMenu( "&Éditer" );
        _select_action = new QAction( "Sélectionnez le mode", this );
        _select_action->setCheckable( true );
        connect( _select_action, SIGNAL( triggered() ), this, SLOT( toggleselectmode() ) );
        //_editmenu->addAction( _select_action );
        //_select_action->addTo( new QToolBar( this ) );
        _editmenu->addAction( "&Remplir automatiquement les postes vides", this, SLOT( scheduleAutoFill() ) );
        _editmenu->addSeparator();
        _add_inchannel_action = new QAction( "Ajouter un port &d'entrée...", this );
        connect( _add_inchannel_action, SIGNAL( triggered() ), this, SLOT( addInput() ) );
        _editmenu->addAction( _add_inchannel_action );
        _add_outchannel_action = new QAction( "Ajouter un port &de sortie...", this );
        connect( _add_outchannel_action, SIGNAL( triggered() ), this, SLOT( addOutput() ) );
        _editmenu->addAction( _add_outchannel_action );
        _rename_input_action = new QAction( "&Renommer le port d'entrée...", this);
        connect( _rename_input_action, SIGNAL( triggered() ), this, SLOT(renameInput()) );
	_editmenu->addAction( _rename_input_action );
        _rename_output_action = new QAction( "&Renommer le port de sortie...", this);
        connect( _rename_output_action, SIGNAL( triggered() ), this, SLOT(renameOutput()) );
        _editmenu->addAction( _rename_output_action );
        _remove_inchannel_action = new QAction( "&Supprimer le port d'entrée...", this );
        connect( _remove_inchannel_action, SIGNAL( triggered() ), this, SLOT( removeInput() ) );
        _editmenu->addAction( _remove_inchannel_action );
        _remove_outchannel_action = new QAction( "&Supprimer le port de sortie...", this );
        connect( _remove_outchannel_action, SIGNAL( triggered() ), this, SLOT( removeOutput() ) );
        _editmenu->addAction( _remove_outchannel_action );

	_viewmenu = menuBar()->addMenu( "&vue" );
        _togglein_action = new QAction( "Masquer la console du port &d'entrée", this );
        connect( _togglein_action, SIGNAL( triggered() ), this, SLOT( togglein() ) );
        _viewmenu->addAction( _togglein_action );
        _toggleout_action = new QAction( "Masquer la console du port &de sortie", this );
        connect( _toggleout_action, SIGNAL( triggered() ), this, SLOT( toggleout() ) );
        _viewmenu->addAction( _toggleout_action );

        _helpmenu = menuBar()->addMenu( "&aider" );
        _helpmenu->addAction( "à propos de &JackMix", this, SLOT( about() ) );
        _helpmenu->addAction( "à propos de &Qt", this, SLOT( aboutQt() ) );
...
/*
        ...
        //_lashclient->setJackName( "JackMix" );
*/
        midiControlSender = new MidiControl::ControlSender("JackMix Console");
