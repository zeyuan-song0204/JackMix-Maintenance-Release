	...
	, _show_value( false )
{
	...
	setMinimumSize( int( w*2.2 ), int( m*3.2 ) );
...
void Slider::paintEvent( QPaintEvent* ) {
	...
	// Rect for the whole bar
	{
		...
		if ( dbtondb( _value ) < 1.0 )
			grad.setColorAt( 1.0, palette().color( QPalette::Base ).dark() );
		// Next soft-fades
		grad.setColorAt( qMax( 0.0, dbtondb( _value )-0.01 ), palette().color( QPalette::Base ).light() );
		if ( dbtondb( _value )+0.01 < 1.0 )
		grad.setColorAt( qMin( 1.0, dbtondb( _value )+0.01 ), palette().color( QPalette::Base ) );
