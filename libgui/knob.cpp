...
void Knob::paintEvent( QPaintEvent* ) {
	...
	p.translate( width()/2.0, height()/2.0 );//位置
	...
	p.setPen( Qt::NoPen );
	{
		...
		grad.setColorAt( 0, QColor(204,204,0) );
		//grad.setColorAt( 1, QColor(255,255,204).dark() );
	...
	}
	{
		...
		/*grad.setColorAt( 0.5, palette().color( QPalette::Highlight ) );
		grad.setColorAt( 0, palette().color( QPalette::Highlight ).light() );*/
		grad.setColorAt( 1, QColor(255,255,51) );
		grad.setColorAt( 0, QColor(153,153,0).light());
		grad.setSpread( QGradient::PadSpread );
		p.setBrush( grad );
	...
	if ( 0<dbmax && 0>dbmin )
		_texts << 0.0;
	QFont small = font();
	small.setPointSizeF( qMax( qreal(5.0), font().pointSizeF()/1.8 ) );
	p.setFont( small );
